class BlueGreenProcess::ChildProcess
  ...
  def be_active
    return self if self.status == PROCESS_STATUS_ACTIVE
  ...
  def be_inactive
    return self if self.status == PROCESS_STATUS_INACTIVE
  ...
  def enforce_to_be_active
    raise 'activeじゃないのにrunできないです' if self.status != PROCESS_STATUS_ACTIVE
  end
end
